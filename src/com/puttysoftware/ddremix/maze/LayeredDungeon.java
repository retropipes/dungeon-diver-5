...
final class LayeredDungeon implements Cloneable {
    ...
    public void writeLayeredDungeon(final XDataWriter writer) throws IOException {
	int y, x, z, e;
    ...
    public static LayeredDungeon readLayeredDungeonV1(final XDataReader reader) throws IOException {
	int y, x, z, e, mazeSizeX, mazeSizeY, mazeSizeZ;
	mazeSizeX = reader.readInt();
	mazeSizeY = reader.readInt();
	mazeSizeZ = reader.readInt();
	final LayeredDungeon lt = new LayeredDungeon(mazeSizeX, mazeSizeY, mazeSizeZ);
	for (x = 0; x < lt.getColumns(); x++) {
	    for (y = 0; y < lt.getRows(); y++) {
		for (z = 0; z < lt.getFloors(); z++) {
		    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
			lt.setCell(DDRemix.getApplication().getObjects().readMazeObject(reader,
    ...
    public void writeSavedDungeonState(final XDataWriter writer) throws IOException {
	int x, y, z, e;
	writer.writeInt(this.getColumns());
	writer.writeInt(this.getRows());
	writer.writeInt(this.getFloors());
	for (x = 0; x < this.getColumns(); x++) {
		...
		for (z = 0; z < this.getFloors(); z++) {
		    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
			this.savedDungeonState.getCell(y, x, z, e).writeMazeObject(writer);
		    }
		}
	    }
	}
    ...
    public void readSavedDungeonState(final XDataReader reader, final int formatVersion) throws IOException {
	int x, y, z, e, sizeX, sizeY, sizeZ;
	sizeX = reader.readInt();
	sizeY = reader.readInt();
	sizeZ = reader.readInt();
	this.savedDungeonState = new LowLevelAMODataStore(sizeY, sizeX, sizeZ, MazeConstants.LAYER_COUNT);
	for (x = 0; x < sizeY; x++) {
	    for (y = 0; y < sizeX; y++) {
		for (z = 0; z < sizeZ; z++) {
		    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
			this.savedDungeonState.setCell(
				DDRemix.getApplication().getObjects().readMazeObject(reader, formatVersion), y, x, z,
				e);
		    }
		}
	    }
