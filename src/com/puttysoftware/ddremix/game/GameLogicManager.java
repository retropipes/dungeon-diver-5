...
public final class GameLogicManager {
	...
	final int curr = DDRemix.getApplication().getMazeManager().getMaze().getStones() * 100;
	final int max = this.stoneCount;
	return "Power: " + curr / max + "%";
    }

    public void updateStoneCount() {
	this.stoneCount = DDRemix.getApplication().getMazeManager().getMaze().getStoneCount();
    }
	...
	try {
	    ...
	    DDRemix.getApplication().showMessage(gameName2 + " on " + gameName1);
